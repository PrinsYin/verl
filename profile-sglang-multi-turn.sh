...
# to change the parameters, modify run_qwen2.5-3b_dapo_multiturn.sh
COMMAND="nohup env EXPERIMENT_NAME=${EXPERIMENT_NAME} bash examples/sglang_multiturn/run_qwen2.5-3b_dapo_multiturn.sh trainer.experiment_name=multiturn-tp-${TP_SIZE}-${TIMESTAMP} actor_rollout_ref.rollout.tensor_model_parallel_size=${TP_SIZE} >> logs/dapo-multiturn-tp-${TP_SIZE}-${TIMESTAMP}.log 2>&1 &"
